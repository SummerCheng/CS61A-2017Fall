...
def move_stack(n, start, end):
    ...
    if n==1:
        ...
        # other = 6 - start - end
        move_stack(n-1,start,i)
        print_move(start,end)
        move_stack(n-1,i,end)
...
def multiple_references_explanation():
    return """That's right. When you use use several references to the same interval, you consider them as different 
    intervals and calculate respectively, therefore, the result will be larger."""
...
def quadratic(x, a, b, c):
    ...
    a,b,e=map(f,(low,up,extrem))
    if extrem>low and extrem<high:
        return interval(min(a,b,e),max(a,b,e))
    else:
