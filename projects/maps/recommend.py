...
def find_centroid(cluster):
    ...
    longitude=[site[0] for site in loc]
    latitude=[site[1] for site in loc]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        rname=restaurant_name(r)
        if r in reviewed:
            rate[rname]=user_rating(user,rname)
        else:
            rate[rname]=predictor(r)
    return rate
    # END Question 9
