...
def distance(city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    lat1, lat2, lon1, lon2 = get_lat(city1), get_lat(city2), get_lon(city1), get_lon(city2)
...
def closer_city(lat, lon, city1, city2):
    ...
    if distance(city1,city) < distance(city2,city):
...
def put_piece(board, max_rows, column, player):
    ...
    row=max_rows-1
    while row>=0 and get_piece(board,row,column)!='-':
        row-=1
    if row==-1:
        return row, board
    else:
